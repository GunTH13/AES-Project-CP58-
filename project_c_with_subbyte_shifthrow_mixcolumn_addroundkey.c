...
int shiftrow();
int mixcolumn();
int roundkey(int indexkey[][four]);
...
int roundkey(int matrixkey[][four]){
	int i, j;
	...
	for(i=0; i<4; i++){
		nextmatrixkey[i][0] = nextmatrixkey[i][0]^rconbox[0][i]^matrixkey[i][0];
	}
	for(i=1; i<4; i++){
		for(j=0; j<4; j++){
...
int addroundkey(){
	...
	for(i=0; i<4; i++){
		for(j=0; j<4; j++){
			cipher[j][i] = matrixkey[j][i]^matrix[j][i];
		}
	}
